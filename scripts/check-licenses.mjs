...
const allow = new Set(
  (
    process.env.LICENSE_ALLOW ||
    "MIT,ISC,BSD-2-Clause,BSD-3-Clause,Apache-2.0,CC0-1.0,Unlicense"
  )
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
...
const deny = new Set(
  (
    process.env.LICENSE_DENY ||
    "AGPL-3.0,AGPL-3.0-only,AGPL-3.0-or-later,GPL-3.0,GPL-3.0-only,GPL-3.0-or-later,LGPL-3.0,SSPL-1.0"
  )
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
...
const failOnUnknown =
  (process.env.LICENSE_FAIL_ON_UNKNOWN || "true") === "true";
...
(async function main() {
  ...
  console.log(
    `Licenses: ${summary.allowed} allowed, ${summary.unknown} unknown, ${summary.denied} denied.`
  );
...
function classify(licenseStr) {
  ...
  const ids = (up.match(/[A-Z0-9.+-]+/g) || []).filter(
    (t) => !["WITH", "AND", "OR"].includes(t)
  );
  if (ids.length === 0) return "unknown";
  const isAllowed = (id) => allow.has(id) || allow.has(id.replace(/\+$/, "")); // tolerate trailing +

  if (hasOR) {
    return ids.some(isAllowed)
      ? "allowed"
      : ids.some((id) => deny.has(id))
        ? "denied"
        : "unknown";
  }
  if (hasAND) {
    return ids.every(isAllowed)
      ? "allowed"
      : ids.some((id) => deny.has(id))
        ? "denied"
        : "unknown";
  }
  return isAllowed(ids[0])
    ? "allowed"
    : deny.has(ids[0])
      ? "denied"
      : "unknown";
...
function toMarkdown(json) {
  ...
  lines.push(
    `Allowed: ${json.summary.allowed}, Unknown: ${json.summary.unknown}, Denied: ${json.summary.denied}\n`
  );
  lines.push("## Findings\n");
  ...
  for (const f of json.findings) {
    lines.push(
      `| ${f.name} | ${f.version} | ${f.license.replace(/\|/g, "/")} | ${f.status} |`
    );
  }
